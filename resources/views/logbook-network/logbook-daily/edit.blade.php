...
@section('title', 'Edit Logbook Network')
...
<div class="container">
    ...
    <div class="card border-o shadow rounded">
        <div class="card-body">
            <form action="{{route('logbook-network-daily.update', $item->id)}}" method="post" id="submit-form">
                @method("PUT")
                @csrf
                <div class="form-row">
                    <div class="form-group col-md-2">
                        <label for="cid">Area Problem</label>
                        <input type="text" value="{{$item->area}}" name="area" class="form-control" required>
                    </div>
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <select class="form-control" name="sites" required>
                            <option value="Bandung" {{$item->sites == 'Bandung' ? 'selected' : ''}} >Bandung
                            </option>
                            <option value="Bogor" {{$item->sites == 'Bogor' ? 'selected' : ''}}>Bogor
                            </option>
                        </select>
                    </div>
