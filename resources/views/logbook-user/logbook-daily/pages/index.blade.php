    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                ...
                <a href="{{route('logbook-user-daily.create')}}" class="btn btn-primary rounded">Add Logbook
                    User</a>
                <a href="{{route('logbook-user-weekly.index')}}" class="btn btn-secondary rounded">To Category
                    Logbook</a>
                <a href="{{ route('report.logbookUserWeekly') }}" class="btn btn-success ml-4  float-right rounded">
                    Report Logbook Weekly
                </a>
                <a href="{{ route('report.logbookUserDaily') }}" class="btn btn-info float-right rounded">
                    Report Logbook
                </a>
            ...
            <div class="card-body">
                ...
                <table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$item->corpnet->cid}}</td>
                            <td>{{$item->corpnet->nama}}</td>
                            <td>{{$item->off_date}}</td>
                            <td>{{$item->off_time}}</td>
                            <td>{{$item->location_site}}</td>
                            <td>{{$item->follow_up_by}}</td>
                            <td>{{$item->impact}}</td>
                            ...
                            <td>
                                <a href="{{ route('corpnet.show', $item->id) }}" class="btn btn-info btn-sm">
                                    <i class="fa fa-address-card-o"></i> Show Detail
                                </a>

                                <a href="{{route('logbook-user-daily.edit', $item->id)}}"
                                    class="btn btn-success btn-sm">Edit</a>
                                <form action="{{route('logbook-user-daily.destroy', $item->id)}}" method="post"
